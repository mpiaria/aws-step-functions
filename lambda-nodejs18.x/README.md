...
```bash
lambda-nodejs18.x$ sam logs -n StockCheckerFunction --stack-name lambda-nodejs18.x --tail
```
...
```bash
lambda-nodejs18.x$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name lambda-nodejs18.x
```
